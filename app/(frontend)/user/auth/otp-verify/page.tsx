...
const OtpVerify = () => {
    ...
    const [phoneNo, setPhoneNo] = useState(''); // To store phone number from query param

    useEffect(() => {
        const phoneFromQuery = searchParams.get('phoneNo');
        if (phoneFromQuery) {
            setPhoneNo(phoneFromQuery);
        } else {
            const storedPhoneNo = localStorage.getItem('user-phone-to-verify'); 
            if (storedPhoneNo) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            localStorage.removeItem('user-phone-to-verify'); // Clear stored phone number
            router.push(`/user/auth/signup?phoneNo=${encodeURIComponent(phoneNo)}`);
            ...
            <div className="w-full max-w-md p-8 rounded-3xl bg-white/40 backdrop-blur-xl border border-purple-200/50 shadow-[0_20px_50px_rgba(139,_92,_246,_0.15)] flex flex-col items-center">
                ...
                <div className="flex flex-col items-center mb-8">
                    ...
                    <p className="text-slate-500 mt-2 text-sm text.center">
                        ...
                        <Link href="/user/auth/phone-verify"> <span className="text-purple-600 font-semibold cursor-pointer hover:underline">Edit number</span></Link>
                    </p>
                </div>
