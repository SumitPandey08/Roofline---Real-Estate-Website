...
const Page: React.FC = () => {
  ...
  const handleSendOTP = async (e?: FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('user-phone-to-verify', fullNumber);
  ...
  const handleVerifyCode = async (e: FormEvent) => {
    ...
    try {
      ...
      localStorage.removeItem('user-phone-to-verify');

      setTimeout(() => {
        router.push(`/user/auth/signup?phoneNo=${encodeURIComponent(fullPhoneNo)}`);
      }, 1500);
    } catch (err) {
